---
id: 3
title: "Русификатор для Don't Sleep with fishes"
author: "Akseleron, Digidro"
description: "Гайд по установке русификатора для игры Don't Sleep with fishes"
image_url: "/guides/dswf-ru/cover.webp"
created_at: "2026-06-23T12:00:00Z"
tags:
  - Игры
---

<div align="center">

![Русификатор для Don't Sleep with fishes](/guides/dswf-ru/title.webp)

</div>

> **Автор перевода:** Akseleron  
> **Текстуры:** Digidro
>
> Вы можете поддержать автора, оставив звезду [проекту на GitHub](https://github.com/Akseleron/dont-sleep-with-the-fishes-russian).

**DSWF Russian Patcher** — это установщик русификации для игры **Don't Sleep With The Fishes**.

Патчер сам устанавливает все необходимые файлы. Вам нужно только выбрать папку с игрой и нажать кнопку установки.

## Скачать патчер: {#download}

- [Windows](https://github.com/Akseleron/dont-sleep-with-the-fishes-russian/releases)
- [Linux](https://github.com/Akseleron/dont-sleep-with-the-fishes-russian/releases) (через Wine)

## Что переводится: {#what-is-translated}

В текущей версии русификатор включает:

- русский текст;
- русские игровые текстуры;
- русификацию меню и интерфейса;
- автоматическое создание резервных копий перед установкой;
- удаление русификации с восстановлением оригинальных файлов.

## Приложение: {#screenshot}

![Как выглядит патчер](/guides/dswf-ru/patcher.webp)

## Установка: {#install}

1. Распакуйте игру.
2. Запустите патчер.
3. Нажмите **Выбрать папку игры**.
4. Укажите папку, в которой находится файл:

   ```text
   DontSleepWithTheFishes.exe
   ```

5. Оставьте включёнными нужные галочки.
6. Нажмите **Установить**.

Патчер проверит выбранную папку и установит русификацию.

## Если русификация уже установлена: {#already-installed}

Если русификация уже стоит, патчер сообщит об этом и предложит варианты:

- переустановить русификацию;
- удалить русификацию и восстановить оригинальные файлы;
- отменить действие.

## Удаление русификации: {#uninstall}

1. Запустите патчер.
2. Выберите папку игры.
3. Нажмите **Удалить русификацию**.
4. Подтвердите восстановление файлов.

Патчер вернёт изменённые файлы из резервной копии.

## Запуск на Windows: {#windows-run}

После установки запускайте игру обычным способом:

```text
DontSleepWithTheFishes.exe
```

## Запуск на Linux / Wine: {#linux-run}

На Linux после установки патчер создаёт файл:

```text
run_dswf_rus.sh
```

Для запуска русифицированной версии используйте его:

```bash
./run_dswf_rus.sh
```

> ⚠️ Не рекомендуется запускать игру напрямую через `wine DontSleepWithTheFishes.exe`, потому что в таком случае перевод может не загрузиться.

## Важное примечание! {#note}

В текущей версии шрифт может выглядеть неидеально в некоторых местах интерфейса. Это не мешает работе русификации, но визуальная полировка шрифта ещё не завершена.

## Поддерживаемая версия игры: {#supported-version}

Русификатор рассчитан на:

```text
Don't Sleep With The Fishes v1.1.2
```

На других версиях игры патчер может работать некорректно.

## Что делать, если что-то пошло не так: {#troubleshooting}

1. Запустите патчер снова.
2. Выберите папку игры.
3. Нажмите **Удалить русификацию**.
4. После восстановления попробуйте установить русификацию заново.

Если игра не запускается или перевод не появился, убедитесь, что выбрана именно папка с `DontSleepWithTheFishes.exe`.
